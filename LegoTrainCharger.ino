...
const int Level_1 = 30;                             // maximum mA difference between current measurements during steady state
...
void loop() {
  ...
  // read battery voltage, and charger current if enabled
  // if ((VbatCap <= BatHigh) && (Charging==1)) {       // if the battery is below 90% and the charging bit is set, then keep charging
    ParknCharge();
  //  }
  //  else {
  //   MotorDrive();
...
void ParknCharge() {
  digitalWrite(ChrgEn, HIGH);  Charging = HIGH;                // enable wireless receiver connection to battery charger and set charging bit to high
  StartLoop = currentTime;                                     // start time of routine, will be used to calculate distance train moves
  adc2 = 0;
  for (int i = 1; i < n; i++) {
    adc2 = adc2 + ads.readADC_SingleEnded(2);
  }   // get latest charger current
  Ichrg = adc2 / Rchrg / n;
  ...
  Iref = Ichrg;                                                     // set reference current to latest measurement

  // calculate which case to use based on Idelta, and absolute current and past Idelta

  if ((-Level_1 <= Idelta) && (Idelta <= Level_1) && (Ichrg < Level_1)) {
    ChrgState = 'z';                                                                      // no measured current change for
  }
  if ((Idelta >= Level_1) && (Ichrg >= Level_1)) {
    ChrgState = 'i';                                                                    // current increasing  as train moves
  }
  if ((Idelta <= Level_1) && (Ichrg >= Level_1)) {
    ChrgState = 'd';                                                                    // current decreasing as train moved
  }
  if ((Idelta >= -Level_1) && (Idelta <= Level_1) && (Ichrg > Level_1)) {
    ChrgState = 'p';                                                                    // current constant as train moves and charger current is high, loop centered
  }

  switch (ChrgState) {
    case 'z':
      if ((CaseWas = 'I') || (CaseWas = 'D') && (Ichrg > Level_1)) {
        analogWrite(MotorAspeed, 0);                               // STOP train, charger appears to be positioned on loop
        Serial.print("Charging loop found, charging at: "); Serial.println(Ichrg);
        CaseWas = 'z';
      }
      break;

    case 'i':
      if ((CaseWas = 'z') || (CaseWas = 'i')) {
        Speed = Speed / 2;                                                                      // if current charger increased sensed, slow train to 1/3 previous
        analogWrite(MotorAspeed, Speed);
      }
      break;

    case 'd':
      if ((CaseWas = 'z') || (CaseWas = 'i')) {                                                  // if charge current is decreasing, reverse train
        if (Forward) {
          Reverse;
        }
        if (Reverse) {
